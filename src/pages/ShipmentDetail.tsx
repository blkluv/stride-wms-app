...
export default function ShipmentDetail() {
  ...
  const handleCompleteOutbound = async () => {
    ...
    if (profile?.tenant_id) {
      try {
        // These are auto-generated by receiving matching and are hidden in the chips UI by default.
        const matchingDiscrepancyCodes = new Set<string>([
          'PIECES_MISMATCH',
          'VENDOR_MISMATCH',
        ...
        const missingCodes: ShipmentExceptionCode[] = (Array.isArray(openExceptionRows) ? openExceptionRows : [])
          ...
          .filter((r: any) => r?.code && !matchingDiscrepancyCodes.has(r.code) && !String(r.note || '').trim())
          .map((r: any) => r.code);
